...
const Cancelride = () => {
    ...
    const handlecancel = () => {
        toast.success('Ride Cancelled Sucessfully ')
    ...
    return (
        <div className=' flex flex-col items-center  p-5 py-10 gap-5   h-[88.5vh] '   >
                ...
                {cancelridedata.map((value) => (
                    <div className='flex flex-col gap-3  ' >
                        <div className='flex gap-2  ' >
                            <input name='option' value={value.id} onChange={(e) => settextareaopen(e.target.value)} type="radio" />
                            <label htmlFor="">
                                <span>{value.id}</span>
                                {value.label}
                            </label>
                        </div>
                        {value.id == "F." && textareaopen == 'F.' ? (<div>
                            <textarea
            ...
            <button onClick={handlecancel} className='w-full  cursor-pointer  rounded-2xl p-2 text-white bg-purple-900'>Continue</button>
